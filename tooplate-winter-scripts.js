...
// If a gallery item wraps a link (e.g. link to a detail page), open it in an iframe inside the lightbox
document.querySelectorAll('.gallery-link').forEach(link => {
   link.addEventListener('click', (e) => {
      e.preventDefault();
      e.stopPropagation();
      const href = link.getAttribute('href');
      openLightboxFrame(href);
   });
});
...
function openLightbox(item) {
   ...
   // ensure any iframe is hidden
   const existingFrame = document.getElementById('lightboxFrame');
   if (existingFrame) {
   ...
   // show navigation for image mode
...
function openLightboxFrame(url) {
   // hide image elements
   ...
   let frame = document.getElementById('lightboxFrame');
   if (!frame) {
   ...
   // hide prev/next for iframe mode
...
document.addEventListener('keydown', (e) => {
   ...
   if (e.key === 'Escape') {
      ...
      // only navigate images when iframe is not active
      const frame = document.getElementById('lightboxFrame');
      if (!frame || frame.style.display === 'none') prevImage.click();
   } else if (e.key === 'ArrowRight') {
      const frame = document.getElementById('lightboxFrame');
      if (!frame || frame.style.display === 'none') nextImage.click();
   }
});
